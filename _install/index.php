...
function check_($check) {
    ...
    $file              = file_get_contents('http://www.localhost/rec/purchase.php?code=' . $check . '&url=' . $siteurl, false, stream_context_create($arrContextOptions));
    if ($file) {
...
function check_success($check) {
    ...
    $file              = file_get_contents('http://www.localhost/rec/purchase.php?code=' . $check . '&success=true&url=' . $siteurl, false, stream_context_create($arrContextOptions));
    if ($file) {
        $check             = json_decode($file, true);
    } else {
        $check             = array('status' => 'SUCCESS', 'url' => $siteurl, 'code' => $check);
...
// | @author_url 1: http://www.localhost/rec
